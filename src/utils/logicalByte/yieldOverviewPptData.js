...
export function createYieldOverviewPptCalculator(itemInfoMap, showSkillT4Materials = false) {
  ...
  const baseOperatorCostTable = createV2OperatorCostTable(operatorTableV2)
  const activeOperatorTable = {
    ...operatorTableV2,
    ...uploadedCharacterData.operatorTable,
  }
  const activeOperatorCostTable = {
    ...baseOperatorCostTable,
...
function createV2OperatorCostTable(operatorTable = {}) {
  const costTable = {}

  for (const [charId, operator] of Object.entries(operatorTable)) {
    costTable[charId] = {
      elite: Array.isArray(operator?.elite) ? operator.elite : [],
      allSkill: Array.isArray(operator?.allSkill) ? operator.allSkill : [],
      skills: (operator?.skills || []).map((skill) =>
        (skill?.skillLevelUpCost || []).map((rank) => {
          const cost = {}
