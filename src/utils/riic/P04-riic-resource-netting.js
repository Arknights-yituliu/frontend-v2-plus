...
function buildNetGoldResource(resourcesByKey, tradingFlowTotals, cycleHours) {
  const normalizedTradingFlowTotals = tradingFlowTotals || {};
  ...
  const consumedPerCycle = Number(
    normalizedTradingFlowTotals.goldConsumptionPerCycle || 0,
  );
  const consumedPerDay = consumedPerCycle * perDayMultiplier;
  const virtualGoldOutputPerCycle = Number(
    normalizedTradingFlowTotals.virtualGoldOutputPerCycle || 0,
  );
...
function buildNetLmdResource(
  ...
  const normalizedManufactureFlowTotals = orundumManufactureFlowTotals || {};
  const tradingLmd = getDirectYieldResource(resourcesByKey, "lmd");
  const roomCount =
    tradingLmd.roomCount +
    Number(normalizedManufactureFlowTotals.roomCount || 0);
  const calculatedRoomCount =
    tradingLmd.calculatedRoomCount +
    Number(normalizedManufactureFlowTotals.calculatedRoomCount || 0);
  ...
  const orundumManufactureConsumptionPerCycle = Number(
    normalizedManufactureFlowTotals.lmdConsumptionPerCycle || 0,
  );
...
function buildNetOriginiumShardResource(
  ...
  const normalizedTradeFlowTotals = orundumTradeFlowTotals || {};
  const manufacturedShards = getDirectYieldResource(
    resourcesByKey,
    "originiumShard",
  );
  const roomCount =
    manufacturedShards.roomCount + normalizedTradeFlowTotals.roomCount;
  const calculatedRoomCount =
    manufacturedShards.calculatedRoomCount +
    normalizedTradeFlowTotals.calculatedRoomCount;
  ...
  const perDayMultiplier = cycleHours > 0 ? 24 / cycleHours : 0;
  const consumedPerCycle = Number(
    normalizedTradeFlowTotals.shardConsumptionPerCycle || 0,
  );
  const consumedPerDay = consumedPerCycle * perDayMultiplier;
