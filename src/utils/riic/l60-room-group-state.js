...
export function createRiicRoomGroupCandidateState({
  ...
  const cohorts = staffingRequirement.cohorts.map((cohort) => {
    ...
    const candidateResolution = resolveRiicRoomCandidateSkeletons({
      ...
      powerPlantCount: layoutData.powerPlantCount,
      tradingStationCount: layoutData.tradingStationCount,
      goldManufactureStationCount: layoutData.goldManufactureStationCount,
      manufactureProductKindCount: layoutData.manufactureProductKindCount,
      facilities: layoutData.facilities,
      trainingMode,
      idealTrainingRaritySelection,
    });
