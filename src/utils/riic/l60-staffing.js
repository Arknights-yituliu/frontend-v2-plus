...
function createCohort({
  ...
  const stationLabel = `站点 ${stationIndexes
    .map((stationIndex) => stationIndex + 1)
    .join("、")}`;
  const cohortId = [
    `level-${stationLevel}-slot-${slotCount}`,
    String(cohortSuffix || "").trim(),
  ]
    .filter(Boolean)
    .join("-");
  const teams = Array.from({ length: resolvedTeamCount }, (_, index) => ({
    ...
    index,
    label: `${stationLabel} ${slotCount}人组 ${index + 1}`,
    stationLevel,
    slotCount,
  }));
...
function splitThreeTimesStationIndexes(stationIndexes, shiftMode) {
  if (shiftMode !== "threeTimes" || stationIndexes.length <= 2) {
    return [stationIndexes];
  }

  const chunks = [];
  for (let index = 0; index < stationIndexes.length; index += 2) {
    chunks.push(stationIndexes.slice(index, index + 2));
  }
  return chunks;
}
...
export function getRiicRoomGroupStaffingRequirement({
  ...
  const cohorts = [...stationIndexesByCohort.values()]
    .flatMap((entry) =>
      splitThreeTimesStationIndexes(
        entry.stationIndexes,
        shiftMode,
      ).map((stationIndexes) =>
        createCohort({
          ...
          shiftMode,
          twoShiftRotationMode,
          cohortSuffix: `stations-${stationIndexes
            .map((stationIndex) => stationIndex + 1)
            .join("-")}`,
          teamCount:
