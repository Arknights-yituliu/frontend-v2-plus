...
async function getItemInfoList(stageConfig, maxIteration = 50, tolerance = 0.000001) {
    ...
    for (const table of COMPOSITE_TABLE) {
      ...
      switch (strategy) {
        ...
        case "WORKSHOP_STRATEGY_COMMON":
          byproductRate = 0.1 * (1 + byproductRateIncrease);
          break;
      }
      ...
      if (resolve) {
        ...
        thisItemValue =
          (targetItemValue + expectedByproductValue * byproductRate + expectedCausalityObtained * causalityValue - lmdCost * lmdValue) / target.count;
      } else {
        // 紫、金材料是向上合成
  ...
  function _updateT3EliteMaterialValue() {
    ...
    for (const [seriesId, { stageId, stageEfficiency }] of maxStageEfficiencyMap) {
      //每次加新材料都在这里
      if (stageEfficiency < 0) {
        itemValueMap.set("31103", 34.1677);
        itemValueMap.set("31113", 31.054);
        continue;
      }
