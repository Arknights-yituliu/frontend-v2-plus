...
function getButshuFamilyConfiguration(candidate) {
  ...
  const group = variantGroupId.slice("family-butshu:".length);
...
export function recalculateRiicButshuCandidate({
  ...
  if (
    ...
    stationLevel < 1 ||
    (candidate?.members || []).length !== configuration.fixedMemberCount
  ) {
    return null;
  }
  ...
  const rawOrderEfficiency =
    (configuration.hasShamare ? 90 : 0) + fallbackPercent;
