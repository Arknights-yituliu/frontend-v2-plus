<template>
  <main class="json-compare-page">
    ...
    <section class="json-compare-upload-grid">
      <article class="json-compare-panel">
        <div class="json-compare-panel-head">
          <div>
            <h2>上传数据</h2>
            <p>{{ datasetState.left.fileName || '未选择文件' }}</p>
          </div>
          <label class="json-compare-upload-button">
            <el-icon><UploadFilled /></el-icon>
            <span>选择 JSON</span>
            <input type="file" accept=".json,application/json" @change="onFileChange('left', $event)" />
          </label>
        </div>

        <div v-if="datasetState.left.error" class="json-compare-error">
          {{ datasetState.left.error }}
        </div>

        <div v-else class="json-compare-summary">
          <div class="json-compare-summary-item">
            <span>识别来源</span>
            <strong>{{ datasetState.left.summary.source }}</strong>
          </div>
          <div class="json-compare-summary-item">
            <span>有效记录</span>
            <strong>{{ datasetState.left.summary.count }}</strong>
          </div>
          <div class="json-compare-summary-item">
            <span>文件大小</span>
            <strong>{{ datasetState.left.fileSize }}</strong>
          </div>
        </div>

        <div class="json-compare-keys">
          <span v-for="key in datasetState.left.summary.keys" :key="key">{{ key }}</span>
          <em v-if="datasetState.left.summary.keys.length === 0">暂无字段</em>
        </div>

        <pre class="json-compare-preview">{{ datasetState.left.preview }}</pre>
      </article>
      ...
      <article class="json-compare-panel">
        <div class="json-compare-panel-head">
          <div>
            <h2>Penguin 缓存</h2>
            <p>{{ datasetState.right.fileName || '正在加载缓存' }}</p>
          </div>
          <el-button :loading="cacheLoading" @click="loadCachedPenguinMatrix">
            <el-icon><RefreshRight /></el-icon>
            <span>刷新缓存</span>
          </el-button>
        </div>

        <div v-if="datasetState.right.error" class="json-compare-error">
          {{ datasetState.right.error }}
        </div>

        <div v-else class="json-compare-summary">
          <div class="json-compare-summary-item">
            <span>缓存来源</span>
            <strong>{{ datasetState.right.summary.source }}</strong>
          </div>
          <div class="json-compare-summary-item">
            <span>有效记录</span>
            <strong>{{ datasetState.right.summary.count }}</strong>
          </div>
          <div class="json-compare-summary-item">
            <span>缓存规模</span>
            <strong>{{ datasetState.right.fileSize }}</strong>
          </div>
        </div>

        <div class="json-compare-keys">
          <span v-for="key in datasetState.right.summary.keys" :key="key">{{ key }}</span>
          <em v-if="datasetState.right.summary.keys.length === 0">暂无字段</em>
        </div>

        <pre class="json-compare-preview">{{ datasetState.right.preview }}</pre>
      </article>
    </section>
...
const penguinCacheStatus = ref('Penguin 缓存：等待加载')
...
const hasContent = computed(() => Boolean(datasetState.left.data || resultRows.value.length > 0))

const statusText = computed(() => {
  if (datasetState.left.error || datasetState.right.error) return 'JSON 解析失败'
  if (canCompare.value) return '上传文件和缓存已就绪'
  if (datasetState.left.data && cacheLoading.value) return '等待 Penguin 缓存'
  if (datasetState.right.data) return '等待上传 JSON 文件'
  if (datasetState.left.data) return '等待 Penguin 缓存'
  return '等待上传 JSON 文件'
})
...
async function loadCachedPenguinMatrix() {
  if (cacheLoading.value) return

  const state = datasetState.right
  cacheLoading.value = true
  state.fileName = 'penguin-cache-v3'
  state.fileSize = '-'
  state.error = ''
  state.preview = '正在加载 Penguin 缓存...'
  state.data = null
  state.summary = createDatasetState().summary
  resultRows.value = []
  resultSummary.value = '等待对比'
  penguinCacheStatus.value = 'Penguin 缓存：加载中'

  try {
    const matrix = await itemCache.getPenguinMatrixCache()
    if (!Array.isArray(matrix)) {
      throw new Error('Penguin 缓存返回数据不是数组')
    }

    state.data = matrix
    state.fileSize = `${matrix.length} 条`
    state.summary = summarizeDropJson(matrix, state.fileName)
    state.preview = createPreview(matrix)
    penguinCacheStatus.value = `Penguin 缓存：${matrix.length} 条`
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error)
    state.error = message
    state.preview = '缓存加载失败'
    penguinCacheStatus.value = 'Penguin 缓存：加载失败'
    ElMessage.error(`Penguin 缓存加载失败：${message}`)
  } finally {
...
function exportResult() {
  ...
  link.href = url
  link.download = `drop-rate-cache-compare-${formatDateForFileName(new Date())}.csv`
