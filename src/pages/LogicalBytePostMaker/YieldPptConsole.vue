...
import YieldPptMaterialRerunBar from '/src/components/logicalByte/YieldPptMaterialRerunBar.vue'
...
import { getActivityAverageStage, getStageData } from '/src/utils/item/stageEfficiencyCal.js'
...
const MATERIAL_DEMAND_MONTHS = 12
...
const materialActivityAverageStageMap = ref(new Map())
const useActivityAverageStageForMaterialPricing = ref(false)
const materialHistoryActStage = ref(getInitialMaterialHistoryActStage())
...
const exportingMaterialRerunIndex = ref(-1)
const materialRerunPreviewIndex = ref(-1)
const materialIconCaptureRefs = ref([])
const materialRerunCaptureRefs = ref([])
const packs = ref([])
...
const cultivationPagePortraitUrls = ref(new Map())
const cultivationPortraitMissingPageIds = ref(new Set())
const hideCultivationSkillIconColumn = ref(false)
...
const materialRerunPreviewOpen = computed({
  get: () => materialRerunPreviewIndex.value >= 0,
  set: value => {
    if (!value) {
      materialRerunPreviewIndex.value = -1
    }
  },
})
const materialRerunPreview = computed(() => materials.value[materialRerunPreviewIndex.value] || null)
const materialDemandSummaries = computed(() => {
  const now = new Date()
  const demandEndDate = new Date(now.getFullYear(), now.getMonth(), 0, 23, 59, 59, 999)
...
const materialRerunOptions = computed(() => materials.value.map(material => getMaterialRerunOptions(material)))
const materialReruns = computed(() => materials.value.map(material => getMaterialRerun(material)))
...
function createMaterial(index) {
  const material = {
    ...
    icon: createImageAsset({
      path: `${ASSET_FOLDER_NAME}/material-${index}-icon.png`,
    }),
    rerun: createImageAsset({
      path: `${ASSET_FOLDER_NAME}/material-${index}-rerun.png`,
    }),
  }
  material.detail.storageKey = createDraftKey('material-detail')
  material.curve.storageKey = createDraftKey('material-curve')
  material.icon.storageKey = createDraftKey('material-icon')
  material.rerun.storageKey = createDraftKey('material-rerun')
  return material
}
...
function createCultivationPage(index) {
  const page = {
    ...
    portrait: createImageAsset({
      path: `${ASSET_FOLDER_NAME}/cultivation-${index}-portrait.png`,
    }),
  }
  page.id = createDraftKey('cultivation-page')
  page.image.storageKey = createDraftKey('cultivation-image')
  page.portrait.storageKey = createDraftKey('cultivation-portrait')
  return page
}
...
function getInitialMaterialHistoryActStage() {
  const history = Array.isArray(TMP_STAGE_RESULT?.historyActStage)
    ? TMP_STAGE_RESULT.historyActStage
    : []

  return [...history].sort((a, b) => getTimestamp(b?.endTime) - getTimestamp(a?.endTime))
}
...
function getMaterialRerunEventKey(event) {
  return `${String(event?.zoneName || '')}@@${getTimestamp(event?.endTime)}`
}

function formatMaterialRerunEvent(event) {
  if (event?.scheduleTime) {
    return `${event.zoneName}（${event.scheduleTime}）`
  }

  const endTime = getTimestamp(event?.endTime)
  const date = endTime > 0 ? new Date(endTime).toLocaleDateString('zh-CN') : ''
  return date ? `${event.zoneName}（${date}结束）` : String(event?.zoneName || '未命名活动')
}

function getScheduledRerunEndTime(scheduleTime) {
  const match = String(scheduleTime || '').match(/(\d{4})\D+(\d{1,2})/u)
  if (!match) {
    return 0
  }
...
function getMaterialRerunEvents(material) {
  const historyByName = new Map(
    materialHistoryActStage.value.map(event => [String(event?.zoneName || ''), event]),
  )

  // Keep the planned rerun order and material drops identical to stageV3's list mode.
  return REPRODUCTION_ACTIVITY
    .map(schedule => {
      const event = historyByName.get(String(schedule?.activityName || ''))
      const endTime = getScheduledRerunEndTime(schedule?.startTime)
      ...
      return {
        ...event,
        zoneName: String(schedule.activityName || event.zoneName || ''),
        endTime,
        scheduleTime: String(schedule.startTime || ''),
      }
    })
    .filter(event => Array.isArray(event?.actStageList)
      && event.actStageList.some(stage => String(stage?.itemSeriesId) === String(material?.seriesId)))
    .sort((left, right) => getTimestamp(left?.endTime) - getTimestamp(right?.endTime))
}

function getMaterialRerunOptions(material) {
  return getMaterialRerunEvents(material).map(event => ({
    value: getMaterialRerunEventKey(event),
    label: formatMaterialRerunEvent(event),
  }))
}

function getMaterialRerun(material) {
  const matchingEvents = getMaterialRerunEvents(material)
  const selectedEventKey = String(material?.rerunEventKey || '')
  const event = matchingEvents.find(candidate => getMaterialRerunEventKey(candidate) === selectedEventKey)
    || matchingEvents.find(candidate => getTimestamp(candidate?.endTime) > Date.now())

  if (!event) {
    return null
  }

  const name = String(event.zoneName || '')
    .replace(/[\s\u00b7\u30fb]*\u590d\u523b\s*$/u, '')
    .trim()
  const stages = event.actStageList
    .filter(stage => Number(stage?.itemRarity) === 3)
    .slice(0, 3)
    .map(stage => ({
      stageCode: String(stage.stageCode || ''),
      itemId: String(stage.itemId || ''),
      stageEfficiency: Number(stage.stageEfficiency || 0),
    }))
    .filter(stage => stage.stageCode && stage.itemId)

  return stages.length > 0
    ? {
      name: name || String(event.zoneName || ''),
      stages,
      endTime: getTimestamp(event.endTime),
    }
    : null
}

function selectMaterialRerun(material) {
  resetAsset(material.rerun)
}

function getMaterialChartTitle(material) {
  return `${material?.title || '\u6750\u6599'}\u8fd1\u4e00\u5e74\u9700\u6c42\u91cf`
}

function formatMaterialDemandCompact(value) {
  const demand = Number(value)
  if (!Number.isFinite(demand)) {
    return '-'
  }
  if (Math.abs(demand) >= 1000) {
    return `${(demand / 1000).toFixed(1)}k`
  }
  return String(Math.round(demand))
}

function getMaterialSummaryText(material, index) {
  const summary = getMaterialDemandSummary(index)
  if (!summary || !material?.title) {
    return material?.title || ''
  }

  return `${material.title} \u603b\u9700\u6c42 ${formatMaterialDemandCompact(summary.totalDemand)}\uff0c\u8fd1\u4e00\u5e74\u9700\u6c42 ${formatMaterialDemandCompact(summary.recentDemand)}`
}
...
function setMaterialIconCaptureRef(index, element) {
  materialIconCaptureRefs.value[index] = element
}

function setMaterialRerunCaptureRef(index, element) {
  materialRerunCaptureRefs.value[index] = element
}

async function capturePngAsset(target, fileName, options = {}) {
  if (!target) {
    return null
  }
...
async function ensureMaterialSupplementalImages() {
  const tasks = []

  materials.value.forEach((material, index) => {
    const summary = getMaterialDemandSummary(index)
    if (summary && !hasAsset(material.icon)) {
      tasks.push({
        asset: material.icon,
        target: materialIconCaptureRefs.value[index],
        fileName: `material-${index + 1}-icon.png`,
      })
    }

    if (materialReruns.value[index] && !hasAsset(material.rerun)) {
      tasks.push({
        asset: material.rerun,
        target: materialRerunCaptureRefs.value[index],
        fileName: `material-${index + 1}-rerun.png`,
      })
    }
  })
  ...
  for (const task of tasks) {
    const file = await capturePngAsset(task.target, task.fileName)
    if (!file) {
      continue
    }
    task.asset.file = file
    task.asset.path = `${ASSET_FOLDER_NAME}/${task.fileName}`
    task.asset.saved = false
  }
}
...
function openMaterialRerunPreview(index) {
  if (!materialReruns.value[index]) {
    ElMessage.warning('请先选择复刻活动，或刷新材料关卡数据后再试')
    return
  }

  materialRerunPreviewIndex.value = index
}
...
async function refreshMaterialStageData() {
  try {
    ...
    const result = await getStageData(stageConfig)
    useActivityAverageStageForMaterialPricing.value = !!stageConfig.useActivityAverageStage
    if (!useActivityAverageStageForMaterialPricing.value) {
      const activityAverageStage = await getActivityAverageStage(stageConfig)
      materialActivityAverageStageMap.value = new Map(activityAverageStage.map(stage => [String(stage.itemSeriesId), stage]))
    } else {
      materialActivityAverageStageMap.value = new Map()
    }
    ...
    if (Array.isArray(result?.historyActStage) && result.historyActStage.length > 0) {
      materialHistoryActStage.value = [...result.historyActStage]
        .sort((a, b) => getTimestamp(b?.endTime) - getTimestamp(a?.endTime))
    }
  } catch (error) {
...
async function generateMaterialRerunImage(material, index) {
  if (exportingMaterialRerunIndex.value !== -1) {
    return
  }
  ...
  const target = materialRerunCaptureRefs.value[index]
  if (!target) {
    ElMessage.error('复刻条尚未准备完成')
    return
  }

  try {
    exportingMaterialRerunIndex.value = index
    const file = await capturePngAsset(target, `material-${index + 1}-rerun.png`)
    if (!file) {
      return
    }

    material.rerun.file = file
    material.rerun.path = `${ASSET_FOLDER_NAME}/material-${index + 1}-rerun.png`
    material.rerun.saved = false
    ElMessage.success(`${material.title || '材料'}复刻条已生成`)
  } catch (error) {
    console.error('生成材料复刻条失败:', error)
    ElMessage.error(error?.message || '生成材料复刻条失败')
  } finally {
    exportingMaterialRerunIndex.value = -1
  }
}
...
function getCultivationPortraitSource(page) {
  const uploadedPortrait = cultivationPagePortraitUrls.value.get(page?.id)
  if (uploadedPortrait) {
...
function setCultivationPortraitMissing(pageId, missing) {
  if (!pageId) {
    return
  }

  const next = new Set(cultivationPortraitMissingPageIds.value)
  if (missing) {
    next.add(pageId)
  } else {
    next.delete(pageId)
  }
  cultivationPortraitMissingPageIds.value = next
}

function isCultivationPortraitMissing(page) {
  return Boolean(page?.operatorId) && cultivationPortraitMissingPageIds.value.has(page.id)
}

function clearCultivationPagePortraitUrl(pageId) {
  const previousUrl = cultivationPagePortraitObjectUrls.get(pageId)
  if (previousUrl) {
    URL.revokeObjectURL(previousUrl)
  }
  cultivationPagePortraitObjectUrls.delete(pageId)

  const next = new Map(cultivationPagePortraitUrls.value)
  next.delete(pageId)
  cultivationPagePortraitUrls.value = next
}

async function refreshCultivationPagePortrait(page) {
  if (!page?.id) {
    return
  }

  const file = await getAssetFile(page.portrait)
  clearCultivationPagePortraitUrl(page.id)
  if (!file) {
    return
  }

  const objectUrl = URL.createObjectURL(file)
  cultivationPagePortraitObjectUrls.set(page.id, objectUrl)
  cultivationPagePortraitUrls.value = new Map(cultivationPagePortraitUrls.value).set(page.id, objectUrl)
}

function selectCultivationOperator(page) {
  resetAsset(page.portrait)
  clearCultivationPagePortraitUrl(page.id)
  setCultivationPortraitMissing(page.id, false)
  syncCultivationOperatorName(page)
}

function selectCultivationPortrait(page, event) {
  selectAsset(page.portrait, event)
  void refreshCultivationPagePortrait(page)
}

function handleCultivationPortraitLoad(page) {
  setCultivationPortraitMissing(page.id, false)
}

function handleCultivationPortraitError(page) {
  setCultivationPortraitMissing(page.id, true)
}
...
function releaseCultivationPortraits() {
  ...
  for (const objectUrl of cultivationPagePortraitObjectUrls.values()) {
    URL.revokeObjectURL(objectUrl)
  }
  cultivationPagePortraitObjectUrls.clear()
  cultivationPagePortraitUrls.value = new Map()
  cultivationPortraitMissingPageIds.value = new Set()
}
...
function getAllAssets() {
  return [
    ...
    ...cultivationPages.value.flatMap(page => [page.image, page.portrait]),
  ]
}
...
function createDraftSnapshot() {
  return {
    ...
    materials: materials.value.map(material => ({
      ...
      rerun: createDraftAssetSnapshot(material.rerun),
    })),
    activityStore: {
    ...
    cultivationPages: cultivationPages.value.map(page => ({
      ...
      caption: page.caption,
      image: createDraftAssetSnapshot(page.image),
      portrait: createDraftAssetSnapshot(page.portrait),
    })),
    cultivationOptions: {
      hideSkillIconColumn: hideCultivationSkillIconColumn.value,
    },
  }
}
  ...
  try {
    ...
    const restoredMaterials = Array.isArray(draft.materials)
      ? draft.materials.slice(0, 3).map((material, index) => ({
        ...
        rerun: restoreDraftAsset(material?.rerun, `${ASSET_FOLDER_NAME}/material-${index + 1}-rerun.png`),
      }))
      : []
    ...
    const restoredCultivationPages = Array.isArray(draft.cultivationPages)
      ? draft.cultivationPages.map((page, index) => ({
        ...
        portrait: restoreDraftAsset(page?.portrait, `${ASSET_FOLDER_NAME}/cultivation-${index + 1}-portrait.png`),
      }))
      : []
    cultivationPages.value = restoredCultivationPages.length > 0 ? restoredCultivationPages : [createCultivationPage(1)]
    hideCultivationSkillIconColumn.value = draft.cultivationOptions?.hideSkillIconColumn === true

    await restoreDraftAssetFiles()
    await Promise.all(cultivationPages.value.map(refreshCultivationPagePortrait))
  } catch (error) {
...
function getMaterialPackageAssets() {
  return [
    ...
    ...getConfiguredCultivationPages().flatMap(page => [page.image, page.portrait]),
  ].filter(hasAsset)
}

function getArchiveFileName(prefix, includeTime = false) {
  const date = new Date()
  ...
  const timestamp = includeTime ? `${month}${day}${hour}${minute}` : `${year}${month}${day}`
...
async function exportMaterialPackage() {
  ...
  try {
    ...
    await ensureMaterialSupplementalImages()
    await saveDraft()
    const zip = new JSZip()
    ...
    saveAs(archive, getArchiveFileName('收益速览素材包', true))
...
function copyCultivationPage(index) {
  ...
  copy.image.file = source.image.file
  copy.image.saved = source.image.saved
  copy.portrait.file = source.portrait.file
  copy.portrait.saved = source.portrait.saved
  ...
  cultivationPages.value.forEach((page, pageIndex) => {
    ...
    renameAssetIndex(page.portrait, `cultivation-${pageIndex + 1}-portrait`)
  })
  void refreshCultivationPagePortrait(copy)
}
...
function removeCultivationPage(index) {
  if (cultivationPages.value.length <= 1) {
    const removedPage = cultivationPages.value[0]
    if (removedPage) {
      clearCultivationPagePortraitUrl(removedPage.id)
      setCultivationPortraitMissing(removedPage.id, false)
    }
    cultivationPages.value = [createCultivationPage(1)]
    return
  }

  const [removedPage] = cultivationPages.value.splice(index, 1)
  if (removedPage) {
    clearCultivationPagePortraitUrl(removedPage.id)
    setCultivationPortraitMissing(removedPage.id, false)
  }
...
async function hydrateManifest(manifest) {
  hideCultivationSkillIconColumn.value = manifest?.cultivationOptions?.hideSkillIconColumn === true
  ...
  const restoredMaterials = Array.isArray(manifest?.materials)
    ? manifest.materials.slice(0, 3).map((material, index) => ({
      ...
      rerun: createSavedAsset(material?.rerunImage, `${ASSET_FOLDER_NAME}/material-${index + 1}-rerun.png`),
    }))
    : []
  materials.value = restoredMaterials.length >= 2 ? restoredMaterials : [createMaterial(1), createMaterial(2)]
  ...
  cultivationPages.value = Array.isArray(manifest?.cultivationPages)
    ? manifest.cultivationPages.map((page, index) => ({
      ...
      portrait: createSavedAsset(page?.portraitImage, `${ASSET_FOLDER_NAME}/cultivation-${index + 1}-portrait.png`),
    }))
    : []
  ...
  materials.value.forEach((material, index) => {
    ...
    material.icon.storageKey ||= `material-${index + 1}-icon`
    material.rerun.storageKey ||= `material-${index + 1}-rerun`
  })
...
function buildManifest() {
  return {
    ...
    materials: materials.value.map((material, index) => ({
      ...
      rerunImage: hasAsset(material.rerun) ? material.rerun.path : '',
      totalDemand: getTotalR3MaterialDemand(getMaterialDemandItemId(material)),
      recentDemand: getMaterialDemandSummary(index)?.recentDemand || 0,
      monthlyDemand: getMaterialDemandSummary(index)?.monthlyDemand || [],
      summaryText: getMaterialSummaryText(material, index),
      rerun: materialReruns.value[index],
    })),
    activityStore: {
    ...
    cultivationPages: getConfiguredCultivationPages().map(page => ({
      ...
      caption: page.caption.trim(),
      image: hasAsset(page.image) ? page.image.path : '',
      portraitImage: hasAsset(page.portrait) ? page.portrait.path : '',
    })),
    cultivationOptions: {
      hideSkillIconColumn: hideCultivationSkillIconColumn.value,
    },
...
<template>
  <main class="yield-ppt-console">
        ...
        <div v-for="(material, index) in materials" :key="index" class="yield-ppt-row yield-ppt-material-row">
          ...
          <label :class="{ 'yield-ppt-asset-ready': hasAsset(material.curve) }">
            ...
            <div v-if="getMaterialDemandSummary(index)" class="yield-ppt-material-demand-summary">
              ...
              <span>近一年 <strong>{{ formatMaterialDemandCount(getMaterialDemandSummary(index).recentDemand) }}</strong></span>
              <v-btn
          ...
          <label :class="{ 'yield-ppt-asset-ready': hasAsset(material.rerun) }">
            <span>复刻条</span>
            <select
              v-model="material.rerunEventKey"
              :disabled="!material.seriesId"
              @change="selectMaterialRerun(material)"
            >
              <option value="">自动匹配未来最近复刻</option>
              <option
                v-for="option in materialRerunOptions[index]"
                :key="option.value"
                :value="option.value"
              >
                {{ option.label }}
            ...
            <div class="yield-ppt-file-control">
              <input type="file" accept="image/png,image/jpeg" @change="selectAsset(material.rerun, $event)">
              <v-btn
                icon="mdi-image-outline"
                size="small"
                variant="text"
                :disabled="!hasAsset(material.rerun)"
                title="预览当前复刻条图片"
                @click.stop="openImagePreview('复刻条图片', material.rerun)"
              />
              <v-btn
                icon="mdi-eye-outline"
                size="small"
                variant="text"
                :disabled="!materialReruns[index]"
                title="预览当前匹配的复刻条"
                @click.stop="openMaterialRerunPreview(index)"
              />
              <v-btn
                icon="mdi-image-sync-outline"
                size="small"
                variant="text"
                :loading="exportingMaterialRerunIndex === index"
                :disabled="!materialReruns[index]"
                title="按当前匹配活动生成复刻条"
                @click.stop="generateMaterialRerunImage(material, index)"
              />
            </div>
            <small v-if="materialReruns[index]" class="yield-ppt-material-rerun-match">
              匹配：{{ materialReruns[index].name }}
            </small>
            <small v-else-if="material.seriesId" class="yield-ppt-material-rerun-empty">
    ...
    <section class="yield-ppt-section">
      <div class="yield-ppt-section-heading">
        <h2>培养成本分页</h2>
        <div class="yield-ppt-cultivation-heading-actions">
          <v-switch
            v-model="hideCultivationSkillIconColumn"
            label="隐藏技能/精二图标列"
            color="primary"
            density="compact"
            hide-details
        ...
        <div v-for="(page, index) in cultivationPages" :key="page.id" class="yield-ppt-row yield-ppt-cultivation-row">
          <label>
            ...
            <el-select
              ...
              placeholder="选择干员"
              @change="selectCultivationOperator(page)"
            >
              <el-option
            ...
            <div
              v-if="isCultivationPortraitMissing(page) || hasAsset(page.portrait)"
              class="yield-ppt-cultivation-portrait-upload"
            >
              <small v-if="isCultivationPortraitMissing(page)">未匹配到立绘</small>
              <input
                type="file"
                accept="image/png,image/jpeg"
                @change="selectCultivationPortrait(page, $event)"
              >
            </div>
          </label>
          <label>
    ...
    <div class="material-curve-capture-layer" aria-hidden="true">
      ...
      >
        <YieldPptMaterialDemandChart
          :points="getMaterialDemandSummary(index)?.monthlyDemand || []"
          :title="getMaterialChartTitle(material)"
        />
      </div>
    </div>

    <div class="material-icon-capture-layer" aria-hidden="true">
      <div
        v-for="(material, index) in materials"
        :key="`material-icon-${index}`"
        :ref="element => setMaterialIconCaptureRef(index, element)"
        class="material-icon-capture"
      >
        <ItemImage
          :item-id="getMaterialDemandSummary(index)?.itemId || ''"
          :size="51"
        />
      </div>
    </div>

    <div class="material-rerun-capture-layer" aria-hidden="true">
      <div
        v-for="(material, index) in materials"
        :key="`material-rerun-${index}`"
        :ref="element => setMaterialRerunCaptureRef(index, element)"
        class="material-rerun-capture"
      >
        <YieldPptMaterialRerunBar :rerun="materialReruns[index]" />
      </div>
    </div>
    ...
    <div class="yield-ppt-cultivation-capture-layer" aria-hidden="true">
      ...
      >
          <YieldOverviewChart
            ...
            :show-portrait-name="false"
            :show-skill-icon-column="!hideCultivationSkillIconColumn"
            @portrait-load="handleCultivationPortraitLoad(page)"
            @portrait-error="handleCultivationPortraitError(page)"
          />
      </div>
    </div>
      ...
      <v-card v-if="materialCurvePreview" class="material-curve-preview-dialog">
        ...
        <v-card-text>
          <YieldPptMaterialDemandChart
            ...
            :title="getMaterialChartTitle(materialCurvePreview)"
          />
        </v-card-text>
      </v-card>
    </v-dialog>

    <v-dialog v-model="materialRerunPreviewOpen" max-width="960">
      <v-card v-if="materialRerunPreview" class="material-rerun-preview-dialog">
        <v-card-title>{{ materialRerunPreview.title || '材料' }}复刻条预览</v-card-title>
        <v-card-text>
          <div class="material-rerun-preview">
            <YieldPptMaterialRerunBar :rerun="materialReruns[materialRerunPreviewIndex]" />
          </div>
        </v-card-text>
      </v-card>
    </v-dialog>
        ...
        <v-card-text>
          <div class="yield-ppt-cultivation-preview">
            <YieldOverviewChart
              ...
              :show-portrait-name="false"
              :show-skill-icon-column="!hideCultivationSkillIconColumn"
              @portrait-load="handleCultivationPortraitLoad(cultivationPreviewPage)"
              @portrait-error="handleCultivationPortraitError(cultivationPreviewPage)"
            />
          </div>
        </v-card-text>
      </v-card>
...
.yield-ppt-cultivation-heading-actions {
  display: flex;
  align-items: center;
  gap: 8px;
}

.yield-ppt-cultivation-heading-actions :deep(.v-switch) {
  min-width: 188px;
}

.yield-ppt-cultivation-portrait-upload {
  display: grid;
  gap: 4px;
}

.yield-ppt-cultivation-portrait-upload small {
  color: #b45309;
}
...
.material-icon-capture-layer,
.material-rerun-capture-layer {
  position: fixed;
  top: 0;
  left: -18000px;
  pointer-events: none;
...
.material-rerun-preview-dialog :deep(.v-card-text) {
  overflow-x: auto;
  padding: 18px;
  background: #eef1f5;
}

.material-rerun-preview {
  width: 850px;
  min-height: 92px;
}

.yield-ppt-material-rerun-match {
  color: var(--muted);
}
