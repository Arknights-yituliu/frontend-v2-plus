...
function getStageCollectByZone() {
  itemCache.getStageInfoCache().then((response) => {
    ...
    for (const stage of response) {
      ...
      console.log(stageCode,zoneName, stageType);
      if (!indexMap.has(zoneName)) {
      ...
      console.log('复刻活动',stageCollect.value.ActRep);
      console.log('活动',stageCollect.value.Act);
      console.log('活动索引',indexMap);
      const index = indexMap.get(zoneName);
      console.log("actIndex", index);
        ...
        <v-tabs-window v-model="stageTypeTabActive">
          <v-tabs-window-item value="ACT">
            <div v-for="(collect, index) in stageCollect.Act" :key="index">
              ...
              <ActionButton v-for="(stage, index) in collect.list" :btn-text="stage.stageCode" :active="stage.active" @click="reviewStageDropDetail(stage)">
              </ActionButton>
            </div>
          </v-tabs-window-item>
        </v-tabs-window>
  ...
  <div class="flex flex-wrap m-12-0">
    <v-card v-for="stage in reviewStageDropDetailList" class="m-4 stage-detail-card" :title="stage.stageCode">
      <v-card-text>
        ...
        <div :id="`line-chart-${stage.stageId}`" class="stage-detail-line-chart"></div>
        ...
        <v-table title="关卡掉落物产出详情">
          ...
          <tbody>
            <tr v-for="item in stage.dropDetail">
              <td>
