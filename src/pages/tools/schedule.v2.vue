...
function chooseScheduleType(type) {
  const {
    room: { trading, manufacture, power },
  } = type;
...
function toNextPlan(index) {
  if (index < 0) selectedPlanIndex.value = scheduleTypeV2.value.planTimes - 1;
  else if (index >= scheduleTypeV2.value.planTimes) selectedPlanIndex.value = 0;
  else selectedPlanIndex.value = index;
...
function chooseRoom(roomType, index) {
  ...
  if ("trading" === roomType || "manufacture" === roomType) {
    selectedProduct.value = getItemIdByProductName(plansTemplate.value[selectedPlanIndex.value].rooms[selectedRoomType.value][selectedRoomIndex.value].product);
  }
...
const roomTypeFilterMap = new Map();

for (const condition of operatorFilterConditionTable.room.conditions) {
  roomTypeFilterMap.set(condition.roomType, condition);
}
...
function commonFilterOperator() {
  ...
  for (const operator of BUILDING_TABLE) {
    ...
    if (filterOperatorList.value[operator.charId]) {
      ...
      result.description += "<br><br>" + `精英${operator.phase}-等级${operator.level} —— ${operator.description}`;
      filterOperatorList.value[operator.charId] = result;
    } else {
      const tmp = JSON.parse(JSON.stringify(operator));
      tmp.description = `精英${tmp.phase}-等级${tmp.level} —— ${tmp.description}`;
      filterOperatorList.value[operator.charId] = tmp;
    }
  }
}
...
function checkRoomDuplicateOperator(charName) {
  if (plansTemplate.value[selectedPlanIndex.value].rooms[selectedRoomType.value][selectedRoomIndex.value].operators.includes(charName)) {
...
function checkRoomMaximum() {
  if (
    plansTemplate.value[selectedPlanIndex.value].rooms[selectedRoomType.value][selectedRoomIndex.value].operators.length >=
    roomSettlementOperatorMaxQuantity[selectedRoomType.value]
  ) {
...
function deleteOperator(charName) {
  plansTemplate.value[selectedPlanIndex.value].rooms[selectedRoomType.value][selectedRoomIndex.value].operators = plansTemplate.value[
    selectedPlanIndex.value
  ].rooms[selectedRoomType.value][selectedRoomIndex.value].operators.filter((e) => {
    return e !== charName;
  });
...
watch(
  () => plansTemplate.value[selectedPlanIndex.value].rooms[selectedRoomType.value][selectedRoomIndex.value].autofill,
  (newVal) => {
    if (newVal && selectedRoomType.value === "dormitory") {
      if (plansTemplate.value[selectedPlanIndex.value].rooms[selectedRoomType.value][selectedRoomIndex.value].operators.length > 0) {
        cMessage(translate("schedule", "schedule.AutofillDormTip"), "warn");
      }
    }
  }
...
<template>
  <div class="schedule-page">
    <div>
      <!--选择基建布局和换班次数-->
      <v-btn color="red" class="m-2" @click="scheduleTypePopupVisible = !scheduleTypePopupVisible">
        {{ translate("schedule", "schedule.InfrastructureLayout") }}
      </v-btn>

      <v-btn color="orange" class="m-2" @click="useLegacyUI()" text="切换到竖版UI"> </v-btn>
      <input type="file" id="scheduleFile" hidden @change="importScheduleByFile()" />
      <!--根据文件导入排版-->
      <v-btn color="primary" class="m-2" @click="chooseFile()">{{ translate("schedule", "schedule.ImportScheduleFile") }} </v-btn>
      <!--下载排版文件-->
    ...
    <v-text-field density="compact" variant="outlined" :label="translate('schedule', 'schedule.IdPlaceholder')" v-model="scheduleImportId">
        ...
        <v-card-text>
          <div class="schedule-set-wrap">
            ...
            <div class="schedule-set-bar">
              ...
              <v-text-field hide-details density="compact" variant="outlined" v-model="scheduleInfo.title"> </v-text-field>
            </div>
            <!--换班表描述-->
            <div class="schedule-set-bar">
              <span>{{ translate("schedule", "schedule.ScheduleDescription") }}</span>
              <v-text-field hide-details density="compact" variant="outlined" v-model="scheduleInfo.description"> </v-text-field>
            </div>
            <!--换班表完成后的描述-->
            <div class="schedule-set-bar">
              <span>{{ translate("schedule", "schedule.Author") }}</span>
              <v-text-field hide-details density="compact" variant="outlined" v-model="scheduleInfo.author"> </v-text-field>
            </div>
            ...
            <div class="schedule-set-bar">
              ...
              <div>
                <v-btn
                  ...
                  v-for="(menu, index) in SCHEDULE_MENU"
                  :key="index"
                >
                  {{ menu.label }}
                </v-btn>
            ...
            <div class="schedule-set-bar">
              ...
              <div>
                <v-btn
                  color="primary"
                  class="m-2"
                  :variant="num === scheduleTypeV2.planTimes ? void 0 : `tonal`"
    ...
    <div class="maa-schedule-wrap-v2">
      <v-card title="排班设置" class="m-8">
        <v-card-text>
          <div class="flex flex-wrap align-center">
            ...
            <!--当前换班表描述-->
            <v-text-field
              :label="translate('schedule', 'schedule.ShiftDescription')"
              hide-details
              density="compact"
              variant="outlined"
              v-model="plansTemplate[selectedPlanIndex].description"
              class="m-4"
            >
            </v-text-field>
            ...
            <!--当前换班表完成后的描述-->
            <v-text-field
              :label="translate('schedule', 'schedule.ShiftDescriptionPost')"
              hide-details
              density="compact"
              variant="outlined"
              v-model="plansTemplate[selectedPlanIndex].description_post"
              class="m-4"
            >
            </v-text-field>
          </div>
          ...
          <div class="schedule-set-bar-short">
            ...
            <div style="width: 60px">
              <v-switch color="success" density="compact" hide-details v-model="plansTemplate[selectedPlanIndex].drones.enable"></v-switch>
            </div>
            <!--换班前后-->
            ...
            <div style="width: 160px">
              <v-btn
                color="primary"
                :variant="'pre' === plansTemplate[selectedPlanIndex].drones.order ? void 0 : `tonal`"
          ...
          <div class="schedule-set-bar-short">
            ...
            <div style="width: 60px">
              <v-switch color="success" density="compact" hide-details v-model="plansTemplate[selectedPlanIndex].Fiammetta.enable"></v-switch>
            </div>
            <span style="width: 80px">{{ translate("schedule", "schedule.Usage") }}</span>
            <div style="width: 160px">
              <v-btn
            ...
            <div style="height: 65px">
              ...
              <OperatorAvatar @click="FiammettaTargetVisible = true" :size="60" :char-id="getCharId(plansTemplate[selectedPlanIndex].Fiammetta.target)">
              </OperatorAvatar>
          ...
          <div class="schedule-set-bar-short">
            ...
            <v-switch color="success" density="compact" hide-details v-model="isPeriod"></v-switch>
            <!--不选择则按班次顺序执行-->
          ...
          <v-card-text>
            <div class="filter-condition-box">
              <div class="condition-bar" v-for="(room, key) in operatorFilterConditionTable" v-show="room.display" :key="key">
                <v-btn :color="room.color" variant="text" size="small" class="m-2" :text="translate('schedule', room.name)"> </v-btn>
                <v-btn
                  v-for="(condition, index) in room.conditions"
                  :key="index"
                  color="primary"
                  :variant="filterBtnStatus(key, condition.label)"
                  size="small"
                  class="m-2"
            ...
            <div class="operator-check-box-group">
              <div
                ...
                @click="
                  setFiammetta('target', operator.name);
                  FiammettaTargetVisible = false;
                "
              >
                <OperatorAvatar :size="60" :char-id="operator.charId"> </OperatorAvatar>
      ...
      <v-card :title="`${roomTypeMap.get(selectedRoomType)}#${selectedRoomIndex + 1}房间设置`" class="m-8">
        <v-card-text>
          <div class="schedule-set-bar-short">
            ...
            <v-switch color="success" density="compact" hide-details v-model="plansTemplate[selectedPlanIndex].rooms[selectedRoomType][selectedRoomIndex].sort">
            </v-switch>
            <!--补满空位-->
          ...
          <div class="schedule-set-bar-short">
            ...
            <v-switch color="success" density="compact" hide-details v-model="plansTemplate[selectedPlanIndex].rooms[selectedRoomType][selectedRoomIndex].skip">
            </v-switch>
            <!--选择产物-->
            <span class="room-set-description">{{ translate("schedule", "schedule.ProductSelection") }}</span>
            <div class="product-image-wrap" @click="setProduct(product.value)" v-for="(product, index) in productTable[selectedRoomType]" :key="index">
      ...
      <div class="room-wrap">
        ...
        <div class="room-wrap-left">
          <div class="copy-btn-wrap"></div>
          <!--    贸易站-->
          <div
            class="room-template trading"
            :id="`trading#${index}`"
            :class="roomSelectedClass('trading', index)"
            v-for="(num, index) in scheduleTypeV2.trading"
            :key="index"
          ...
          <!--制造站-->
          <div
            class="room-template manufacture"
            :id="`manufacture#${index}`"
            :class="roomSelectedClass('manufacture', index)"
            v-for="(num, index) in scheduleTypeV2.manufacture"
            :key="index"
          ...
          >
            ...
            <div class="settlement-operator">
              <OperatorAvatar
                :size="44"
                v-for="(charName, index) in getRoomOperators('manufacture', index)"
                :key="index"
                :char-id="getCharId(charName)"
                class="m-4"
              >
          ...
          <!--  发电站-->
          <div
            class="room-template power"
            :id="`power#${index}`"
            :class="roomSelectedClass('power', index)"
            v-for="(num, index) in scheduleTypeV2.power"
            :key="index"
          ...
          >
            ...
            <OperatorAvatar
              :size="44"
              v-for="(charName, index) in getRoomOperators('power', index)"
              :key="index"
              :char-id="getCharId(charName)"
              class="m-4"
            ></OperatorAvatar>
          ...
          <div class="copy-btn-wrap">
            <span>
              {{ translate("schedule", "schedule.TempPlan") }}&nbsp;
              {{ translate("schedule", "schedule.Shift") }}
              {{ tmpPlanData.index }}
            </span>
            <v-btn color="primary" @click="copyPlan()" :text="translate('schedule', 'schedule.CopyPlan')"> </v-btn>
            <v-btn color="primary" @click="pastePlan()" :text="translate('schedule', 'schedule.PastePlan')"> </v-btn>
          </div>
        </div>

        <div class="room-wrap-center">
          <!--     控制中枢-->
          <div class="room-template control" :id="`control#0`" :class="roomSelectedClass('control', 0)" @click="chooseRoom('control', 0)">
          ...
          <!--     宿舍-->
          <div
            class="room-template dormitory"
            :id="`dormitory#${index}`"
            :class="roomSelectedClass('dormitory', index)"
            v-for="(num, index) in scheduleTypeV2.dormitory"
            :key="index"
          ...
          >
            ...
            <OperatorAvatar
              :size="44"
              v-for="(charName, index) in getRoomOperators('dormitory', index)"
              :key="index"
              :char-id="getCharId(charName)"
              class="m-4"
            >
        ...
        <div class="room-wrap-right">
          ...
          <!--     会客室-->
          <div class="room-template meeting" :id="`meeting#0`" :class="roomSelectedClass('meeting', 0)" @click="chooseRoom('meeting', 0)">
            <div class="room-name">{{ translate("schedule", "schedule.ReceptionRoom") }}</div>
            <OperatorAvatar
              :size="44"
              v-for="(charName, index) in getRoomOperators('meeting', 0)"
              :key="index"
              :char-id="getCharId(charName)"
              class="m-4"
            ></OperatorAvatar>
          ...
          <!--      加工站-->
          <div class="room-template processing" :id="`processing#0`" :class="roomSelectedClass('processing', 0)" @click="chooseRoom('processing', 0)">
          ...
          <!--     办公室 -->
          <div class="room-template hire" :id="`hire#0`" :class="roomSelectedClass('hire', 0)" @click="chooseRoom('hire', 0)">
            <div class="room-name">{{ translate("schedule", "schedule.Office") }}</div>
            <OperatorAvatar
              :size="44"
              v-for="(charName, index) in getRoomOperators('hire', 0)"
              :key="index"
              :char-id="getCharId(charName)"
              class="m-4"
            ></OperatorAvatar>
      ...
      <div class="schedule-set-bar" style="justify-content: center">
        ...
        <v-btn
          color="primary"
          :variant="index === selectedPlanIndex ? void 0 : `tonal`"
          size="small"
          v-for="(num, index) in scheduleTypeV2.planTimes"
          :key="index"
      ...
      <div class="room-set-wrap">
        <div class="room-set">
          ...
          <div class="selected-operator-wrap">
            <div class="room-operator" v-for="(charId, index) in getRoomOperators(selectedRoomType, selectedRoomIndex)" :key="index">
              <OperatorAvatar :size="40" :char-id="getCharId(charId)"> </OperatorAvatar>
              <v-icon icon="mdi-close" size="16" @click="deleteOperator(charId)" class="operator-delete-icon"></v-icon>
            </div>
          ...
          <v-btn color="primary" variant="tonal" @click="copyOperatorList()" :text="translate('schedule', 'schedule.Copy')"> </v-btn>
          <v-btn color="primary" variant="tonal" @click="pasteOperatorList()" :text="translate('schedule', 'schedule.Paste')"> </v-btn>
          <!--复制的干员-->
          <span class="room-set-description">{{ translate("schedule", "schedule.OperatorsClipboard") }}</span>
          <div class="selected-operator-wrap">
            <OperatorAvatar v-for="(charId, index) in tmpOperatorList" :key="index" :size="40" :char-id="getCharId(charId)"> </OperatorAvatar>
        ...
        <!--筛选干员-->
        <div class="filter-condition-box">
          <div class="condition-bar" v-for="(conditionType, key) in operatorFilterConditionTable" v-show="conditionType.display" :key="key">
            <span :style="`color:${conditionType.color}`">{{ translate("schedule", conditionType.name) }}</span>
            <v-btn
              v-for="(condition, index) in conditionType.conditions"
              :key="index"
              size="small"
              class="m-2"
              color="primary"
        ...
        <v-text-field
          hide-details
          density="compact"
          :label="translate('schedule', 'schedule.SearchInputTip')"
          variant="outlined"
          width="800"
          class="m-8"
        ...
        >
          <template v-slot:append>
            ...
            <v-switch
              ...
              label="仅搜索技能名称"
              density="compact"
              v-model="searchOperatorOnlyBuffName"
              @change="searchOperatorDebounce()"
              hide-details
            >
            </v-switch>

            <v-btn color="primary" text="隐藏未招募干员" @click="filterOperatorByOwn()"> </v-btn>
        ...
        <div class="operator-check-box-group">
          <div class="operator-check-box-option-v2" v-for="(operator, charId) in filterOperatorList" :key="charId">
            <OperatorAvatar @click="chooseOperator(operator.name)" :size="60" :char-id="operator.charId" class="m-a"> </OperatorAvatar>
            <div class="operator-check-label-v2">{{ operator.name }}</div>
